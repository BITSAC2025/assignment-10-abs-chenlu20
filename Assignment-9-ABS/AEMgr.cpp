...
AEState AbstractExecutionMgr::test5()
{
    ...
    // field access: f0 at offset 0
    as[q] = AddressValue(getGepObjAddress("malloc1", 0));
    as.storeValue(q, IntervalValue(10, 10));
    
    // field access: f1 at offset 1
...
AEState AbstractExecutionMgr::test6()
{
    ...
    // condition true: a > 10 => a == 11
    AEState br_true = as;
    IntervalValue a_true = br_true[a].getInterval();
    a_true.meet_with(IntervalValue(11, 11));
    br_true[a] = a_true;
    br_true[b] = br_true[a].getInterval();

    // condition false: a <= 10 => a in [5,10]
    AEState br_false = as;
    IntervalValue a_false = br_false[a].getInterval();
    a_false.meet_with(IntervalValue(5, 10));
    br_false[a] = a_false;

    // join branches
    as = br_true;
    as.joinWith(br_false);
    //@}
...
AEState AbstractExecutionMgr::test8()
{
    ...
    for (u32_t cnt = 0; cnt < widen_delay; cnt++) {
        body_as = head_as;
        body_as[x] = body_as[x].getInterval() + IntervalValue(-1, -1);

        AEState join_state = entry_as;
        join_state.joinWith(body_as);
        head_as = join_state;
    }

    // apply widening twice
    body_as = head_as;
    body_as[x] = body_as[x].getInterval() + IntervalValue(-1, -1);
    AEState join_state = entry_as;
    join_state.joinWith(body_as);
    head_as = head_as.widening(join_state);
